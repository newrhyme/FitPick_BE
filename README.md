...
- AI 가상 피팅 요청 / 결과 조회 / 목록 조회 (비동기 처리)
...
- 주문 상태 변경 (PAID → PREPARING → READY → PICKED_UP) — 변경 시 알림 comment 필수
- 관리자 주문 취소
- 상태 변경 시 알림 자동 생성 (READY는 PICKUP_READY) + FCM 푸시 자동 발송 (토큰 등록된 사용자 한정)

### AI Virtual Try-On

- 사용자의 전신 사진 + 상품 이미지를 입력으로 AI 합성
- **OpenAI gpt-image-1 실제 연동** (출력 1024x1024)
- 배경 제거 + 상황/분위기(TPO)에 맞는 새 배경 자동 생성
- 선택 입력 `style` (자유 텍스트, 예: "바닷가", "카페에서 친구 만나는 자리") — 자동 TPO 매핑보다 우선 적용
- 생성된 결과 이미지를 S3에 업로드해 영구 URL로 관리
- 상태 관리: `PENDING` / `PROCESSING` / `DONE` / `FAILED`
- **비동기 처리** — 요청 즉시 `PROCESSING`으로 응답 후 전용 스레드풀(`tryOnExecutor`)에서 백그라운드 실행
- 완료/실패 시 FCM 푸시 + 인앱 알림 자동 발송 (`TRY_ON_DONE` / `TRY_ON_FAILED`)
...
- **Spring Boot ↔ OpenAI**: 가상 피팅 요청 시 gpt-image-1 호출 → 응답 이미지를 다시 S3에 업로드. 호출은 전용 스레드풀(`tryOnExecutor`)에서 **비동기**로 수행되어 사용자 요청은 즉시 `PROCESSING` 상태로 응답.
- **Spring Boot ↔ FCM**: 주문 상태가 변경되거나 가상 피팅이 완료/실패하면 Firebase Admin SDK를 통해 등록된 디바이스 토큰으로 푸시 발송.
...
| `notifications` | 인앱 알림 (PICKUP_READY / TRY_ON_DONE / TRY_ON_FAILED). `try_on_id` FK + `image_url` 보유 |
...
| Method | Path | 설명 |
| --- | --- | --- |
| GET   | `/api/v1/notifications` | 내 알림 목록 (`isRead=false`만 반환) |
...
10. **주문 상태를 READY로 변경** — `comment` 입력 → 인앱 알림 생성 + FCM 푸시 발송 트리거
11. **CUSTOMER가 인앱 알림 및 FCM 푸시 수신** — 단건/전체 읽음 처리 가능
...
- **가상 피팅 비동기 파이프라인**
  컨트롤러는 `PROCESSING` 행만 `REQUIRES_NEW`로 즉시 커밋하고 클라이언트에는 곧바로 응답. 실제 OpenAI 호출·S3 업로드·DB 업데이트·알림 발송은 전용 스레드풀(`tryOnExecutor`, core=2 / max=4 / queue=20)에서 백그라운드 수행. 완료 시 `TRY_ON_DONE`, 실패 시 `TRY_ON_FAILED` 알림 + FCM 푸시를 보내 프론트가 폴링 없이도 결과를 받을 수 있게 설계.

- **알림 도메인 확장**
  단순 목록만이 아니라 `unread-count`(레드닷), `read`(단건), `read-all`(일괄) API를 분리. 알림 테이블에 `try_on_id` FK와 `image_url`을 두어 가상 피팅 결과 이미지 같은 컨텍스트를 알림 페이로드에 함께 전달.
